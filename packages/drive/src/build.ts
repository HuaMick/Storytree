...
// `--real`). Re-exported from ONE narrow subpath (`@storytree/drive/build`, and `@storytree/cli/build`
// for back-compat) so the studio imports a single build surface, and the worker routes a unit id to
// the right entry by its tier. ADR-0004: the FRONTEND imports NONE of this — only the server-process
// worker does, lazily.
