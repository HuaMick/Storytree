// The build worker — RELOCATED into @storytree/drive (ADR-0133 d.3 / capability `worker-relocation`).
//
// WHY IT LIVES HERE: an app may not import another app's server (ADR-0100), so the desktop local
// backend — which mounts /api/chat but had build DISABLED — could not reach this machinery while it
// lived in apps/studio/server. This module MOVES the worker DOWN into the shared @storytree/drive
// package both surfaces may import: the studio re-points its importers here, and the desktop
// (capabilities 2-3) mounts /api/build + the chat accept→dispatch over the SAME relocated worker.
//
// THE WALL (the relocation's reason-to-exist, ADR-0100): this module imports NOTHING from apps/* —
// only node:crypto (the registry) + locally-declared structural types (the build entries it drives
// are INJECTED via RoutedBuildDeps / BuildContext, never imported from a surface). That is the
// property build-worker-relocation.test.ts asserts structurally (`wr-imports-nothing-from-apps`) and
// what makes the desktop mount legal.
//
// WHAT MOVED (verbatim, ADR-0133 d.3 "reuse the ADR-0090 worker"): the BuildRegistry (the in-memory
// run lifecycle), the runBuildJob / routedBuildRunner / buildRunnerFromNodeBuild /
// adoptRunnerFromAdoptStory worker family, the dispatchAcceptedBuild chat dispatch (ADR-0108 d.3), and
// the BuildContext seam (lifted out of apps/studio/server/apiRouter.ts). Behaviour is unchanged — only
// the home. The spine inside runBuildJob still observes RED→GREEN and SIGNS; this module holds no
// signing key and no verdict path (ADR-0091) — a SAFE build INTENT only.

import { randomUUID } from 'node:crypto';

// ── the build-run registry (ADR-0090 Phase 1 "the local loop") ───────────────
//
// WHAT IT IS: a small state organ. It holds the live build runs the worker drives, captures the
// COARSE progress each build emits (phase/status/verdict LINES, never the raw model stream), and
// records the terminal envelope. It owns NO proof logic — `nodeBuild` → `proveUnit` (the
...
// ── the build worker (ADR-0090 Phase 1 — the single orchestrator boundary) ───
//
 ...
 * declared locally so this module needs no import of `@storytree/drive`'s build entries (which would
 * pull the agent into a surface's module graph). The production adapter coerces the real `Envelope`
 * to this shape.
 */
...
// ── the build seam + the chat-surface dispatch (ADR-0090 / ADR-0108 d.3) ─────
//
// The BuildContext was lifted out of apps/studio/server/apiRouter.ts in the relocation so both the
// studio route table and the desktop mount inject the SAME shape (ADR-0090's single boundary, two
// callers). `dispatchAcceptedBuild` is the mechanism the human's UI click drives after accepting a
// proposed unit id from the chat agent (chat-drive-bridge, ADR-0108 d.3): it reuses the EXISTING
// worker machinery (`createRun` → `runBuildJob`) exactly as `handleBuild`'s POST branch does — the
// DIFFERENCE is shape, not behaviour: a plain function returning a typed result the chat surface folds
// into its stream, rather than an HTTP handler.
//
...
// exit codes and signs; CI re-proves green before trunk (ADR-0022). This module holds no signing key
// and no DB connection.

/** The build seam injected into a route table: the run registry, the build runner, and discovery. */
