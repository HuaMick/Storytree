...
export interface SpawnSurfaceDeps {
   ...
   * to advertise is dropped — a builder drives the whole unit's proof, not a scoped edit).
   */
  spawnBuilder: (
    args: { unitId: string },
    onTrace: (msg: unknown) => void,
...
export function buildSpawnTools(deps: SpawnSurfaceDeps) {
  ...
  const spawnBuilderTool = tool(
    ...
    "Spawn a write-scoped builder session to implement a capability or fix an issue by driving " +
      ...
      "source scope — NOT in this chat. A builder has NO per-run scope knob — it drives the whole " +
      "unit's registered proof, not a scoped edit.",
    {
