...
- **Cold-rebuild = the health invariant (ADR-0010 §6):** a **story** is `healthy` iff a cold agent — given the story's own spec **plus the documented interfaces of its upstream stories** (never their internals) — can drive it red→green. (Distinct from the DAG-stabilisation sense of convergence — open-q §4.)
