...
- **Per-action approval is first-class** (inverts v1's `--dangerously-skip-permissions`): approve / reject / steer individual pi actions in-loop; approvals + steering are typed `actor=operator` events.
