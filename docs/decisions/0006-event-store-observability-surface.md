...
- **Two ingest channels only:** pi's lifecycle stream + `edit` diffs (via `pi-adapter`); orchestrator control-flow events. **No** Claude hooks, **no** OTel-from-an-agent, **no** MCP, **no** trace SaaS.
...
- `pi-adapter` is the **sole** producer of pi-derived events, and the mapping is load-bearing (not a shim).
