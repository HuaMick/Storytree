...
# Relevant ADRs: ADR-XXXX (Library Review mode) — pending allocation (the orchestrator is reserving it).
...
## The model (from ADR-XXXX — pending allocation)
...
- **Block-position anchor replaces the text-quote anchor.** A comment's anchor records WHICH block
  ...
  popover) is REMOVED, not kept alongside — a clean swap (ADR-XXXX; the owner wants the dead code
