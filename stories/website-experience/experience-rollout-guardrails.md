...
# orchestrator-supplemented GLUE; the site-side markers themselves (`data-experience-skip`,
# `data-experience-fallback` on the entry) land WITH the storm/inflection caps — this cap ships the
# WALL, those caps satisfy it. install: true (the suite runs under @storytree/cli; fresh-worktree
...
> **Proof status (honest) — NOT BUILT, `proposed`.** This precedes the code. The rollout decision
> makes real visitors hit a half-built experience from increment D onward; ADR-0134 §4 makes the
> fallback + skip "prerequisites, not polish." A prerequisite enforced only by attestation would rot
> — this capability turns the two obligations that ARE mechanically checkable into a refusing gate
> (the `check:web-grounding` / `check:web-engine` precedent: parent-side checks over the checked-out
> `web/` submodule already run in `pnpm gate`). The ADEQUACY of the exits (does the fallback read as
> a real calm view; does the skip land well) stays human — story-UAT leg 3 — the gate guards
> PRESENCE and the no-WebGL floor, which is exactly what a machine can honestly hold.
...
1. **`erg-skip-marker-required`** — the storm is never a toll booth
   ...
   - **covers —** `packages/cli/src/web-experience-check.ts` *(provisional path)*
