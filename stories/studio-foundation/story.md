...
- `browse-library` → `seed-library-corpus`
  - browse-library consumes the **artifacts the seeder wrote**: the grid, chip counts (definition 53, pattern 11, guardrail 8, principle 5, techstack 4), and every doc: citation come from apps/studio/data/assets.json, which exists only because seed.assets.mjs produced it — a data-provenance coupling on the seeder's output file.
...
7. Navigate to #/library. **Success —** the grid renders the seeded corpus with the 'all (81)' chip and one live-count chip per non-empty category (definition 53, pattern 11, guardrail 8, principle 5, techstack 4) — the artifacts the seeder wrote, served from assets.json.
...
13. Clean up: delete the probe comment so the git-tracked stores return to their seeded baseline (comments.json `[]`, assets.json its 81 records). **Success —** the working tree is clean — the UAT left no residue.
