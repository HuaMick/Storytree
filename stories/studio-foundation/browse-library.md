...
> **Proof status (honest) —** CODE EXISTS AND RUNS, NO AUTOMATED PROOF YET. All read-side behaviours are implemented and the studio runs under `pnpm --filter studio dev` serving the real 81-artifact seed; the full UAT path (open #/library → narrow by chip → substring-search → open #/asset/<id> → follow a doc: ref to #/doc/<relpath>) is manually walkable today. But apps/studio has NO automated test suite and NO scripted UAT: none of the 11 contracts exist as tests, and the UAT is unscripted. The corpus counts and the 0-asset:-refs honesty caveat were verified by inspecting apps/studio/data/assets.json during this decomposition, not by any committed assertion. RETROSPECTIVE spec — nothing here is 'proven' or 'healthy', only present and exercised by hand.

## Guidance

Routing is a dependency-free custom hash router (route.ts): doc/asset ids are URI-encoded into a single hash segment so slashes in doc relpaths (e.g. 'decisions/0002-...md') don't break parsing — RefLink's doc link goes to #/doc/<encoded relpath>, which is why the citation-follow leg needs read-corpus real. Both the category-filter and the substring-search live in ONE useMemo over the same assets array (Library.tsx:14-24); the search is literal String.includes over a lowercased `id title description body` haystack (Library.tsx:19) — describe it at that fidelity, NOT as ranked relevance. Chip counts are computed inline by re-filtering the assets array per category (Library.tsx:48), and a category with n===0 renders no chip (Library.tsx:49) — the same per-category-count surface is mirrored in Sidebar.tsx:26-41, so they share the live counts. The 5-category taxonomy is a closed schema, not a unit: AssetCategory union + ASSET_CATEGORIES order + ASSET_CATEGORY_GLOSS map (types.ts:67-72,124-139) — chips/gloss/route-guard all consume it but it is observed only THROUGH them. RefLink resolution is NOT live re-validation: it reads docIds/docTitles that App.tsx:58-59 derives once from the doc index into the shared AppData context (appData.ts:8-16); an unknown ref degrades gracefully to a muted '(unknown doc/asset)' span. SEED REALITY (verified against apps/studio/data/assets.json): exactly 81 artifacts (definition 53, pattern 11, guardrail 8, principle 5, techstack 4), 80 carry references, 127 doc: refs, and 0 asset: refs — so the asset: branch of RefLink (AssetView.tsx:114-122) is real but DEAD under the seed; the honest minimal UAT must follow a doc: ref (e.g. artifact 'deep-modules' → 'doc:decisions/0002-work-hierarchy-story-capability-contract.md').
...
1. Start the studio (pnpm --filter studio dev) and open #/library. Observe the grid renders with the 'all (81)' chip active, plus one chip per non-empty category showing its live count: definition (53), pattern (11), guardrail (8), principle (5), techstack (4). No category gloss banner is shown yet (category is null).
2. Click the 'definition' category chip. The route becomes #/library/definition, the grid narrows to the 53 definition artifacts, the 'definition' chip is now active, and the one-line gloss banner appears reading 'definition — what something is'.
3. Type a substring known to occur in one artifact's id/title/description/body (e.g. 'deep') into the search box. The grid narrows further to only the cards whose combined id+title+description+body contains that substring (case-insensitive). Then type a string that matches nothing (e.g. 'zzzqqq') and confirm the 'No artifacts match.' empty state replaces the grid; clear it to restore the list.
4. Click an artifact card known to carry a doc: reference (e.g. 'deep-modules'). The route becomes #/asset/deep-modules and the detail view renders: the category chip + its gloss in the header, the description lede, the markdown-rendered body, and a 'References' list containing the cited ADR rendered as an in-app link showing the doc's title (not the raw 'doc:decisions/0002-...md' string).
5. Click that doc reference link. The route becomes #/doc/decisions%2F0002-...md and DocView loads and renders that ADR as markdown in-app — proving the citation resolved to a real, reachable corpus document. (Honesty: the seed contains 0 asset: references, so this minimal walkthrough deliberately follows a doc: reference; the asset: branch of RefLink is real code but unexercised by the seed.)
