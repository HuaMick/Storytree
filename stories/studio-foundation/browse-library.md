...
collaborators** — the real 81-artifact `seed-library-corpus` output (the grid, chip
...
1. Start the studio (pnpm --filter studio dev) and open #/library. Assert the grid renders with the 'all (81)' chip active, plus one chip per non-empty category showing its live count: definition (53), pattern (11), guardrail (8), principle (5), techstack (4). No category gloss banner is shown yet (category is null).
2. Click the 'definition' category chip. Assert the route becomes #/library/definition, the grid narrows to the 53 definition artifacts, the 'definition' chip is now active, and the one-line gloss banner appears reading 'definition — what something is'.
