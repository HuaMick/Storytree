...
2. Run `node apps/studio/data/seed.assets.mjs` and assert stdout prints `wrote 81 artifacts → …/assets.json` followed by `by category: {"principle":5,"pattern":11,"definition":53,"techstack":4,"guardrail":8}` (categories summing to 81).
3. Read apps/studio/data/assets.json and assert it parses as a JSON array of 81 objects, each carrying id, category (one of principle/pattern/guardrail/techstack/definition), title, description, body, references, createdAt, updatedAt.
...
5. Assert the glossary auto-extraction: a 'definition' artifact such as 'node' exists, cites 'doc:glossary.md', and additionally carries the per-mention ADR refs found in its body (doc:decisions/0004-…, doc:decisions/0009-…); and that 52 of the 53 definitions carry the doc:glossary.md ref (the 53rd, 'proof-mode', is curated).
6. Assert no artifact derived from the v1→v2 term-map table exists (the table after the '## v1 → v2 term map' marker was cut before extraction) and that every reference string is doc:-prefixed (80 of 81 artifacts have >=1 ref; only 'edit-first-curation' has none).
...
8. Re-run `node apps/studio/data/seed.assets.mjs --force` and assert it regenerates the byte-identical 81-artifact corpus, proving --force overrides the guard and the output is deterministic (fixed STAMP timestamps).
